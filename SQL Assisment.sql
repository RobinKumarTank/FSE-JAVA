...
Q2 Ans: select distinct project from EmployeeSalary;
...
Q9 Ans: select Salary + Variable as TotalSalary from EmployeeSalary;
...
Q18 Ans: select concat(EmpId, ManagerId) as ID from EmployeeDetails;

Q19 Ans: select mid(fullname,1,locate('',fullname))as FullName from EmployeeDetails;

Q20 Ans: 	 Select upper(FullName),lower (city) from employeedetails;
...
Q26 Ans: select EmpId from EmployeeDetails where year(DateOfJoining) = 2020;
